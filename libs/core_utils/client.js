...
module.exports = function ({ sock, store, config }) {
   ...
   const client = Object.defineProperties(sock, {
      ...
      sendMedia: {
         async value(jid, url, quoted = "", options = {}) {
            ...
            if (size > 45000000) data = { document: buffer, mimetype: mime, fileName: options?.fileName ? options.fileName : `${sock.user?.name} (${new Date()}).${ext}`, ...options }
            else if (options.asDocument) data = { document: buffer, mimetype: mime, fileName: options?.fileName ? options.fileName : `${sock.user?.name} (${new Date()}).${ext}`, ...options }
      ...
      sendGroupV4Invite: {
         async value(jid, groupJid, inviteCode, inviteExpiration, groupName, jpegThumbnail, caption = "Invitation to join my WhatsApp Group", options = {}) {
            const media = await prepareWAMessageMedia({ image: (await Func.getFile(jpegThumbnail)).data }, { upload: sock.waUploadToServer })
