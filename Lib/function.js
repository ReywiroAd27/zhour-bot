...
module.exports = new class Function{
   ...
   modeFor(mode, m, config) {
     const isPrivate = !m.isGroup
       if (mode.groups === true && mode.privates === true) {
         return true
       } else if (mode.groups === true && mode.privates === false) {
         if (isPrivate) {
           return false
         } else {
           return true
         }
       } else if (mode.groups === false && mode.privates === true) {
         if (m.isGroup) {
           return false
         } else {
           return true
   ...
   permissionLevel(permission, m, db, config) {
     const Status = config.options.status
     if (Status[permission]) {
       if (Status[permission] === 'all') {
         return true
       } else if (Status[permission] === 'owner') {
         if (m.isOwner) {
           return true
         } else {
           return false
         }
       } else if (Status[permission] === 'admin') {
         if (m.isAdmin || m.isOwner) {
           return true
         } else {
           return false
         }
       } else if (Status[permission] === 'botAdmin') {
         if (m.isBotAdmin) {
         ...
         if (db.users[m.sender].status[Status[permission]] === true) {
           return true
         } else {
           return false
         }
   ...
   toPercent(numb) {
     const math = numb * 100
     const result = `${math.toFixed(2)}%`
     return result
   }

    similarityCmd(cmdList, cmd, m) {
     if (cmdList.includes(cmd)) {
        let text = '\n\nI found a command with a closely matching resemblance:\n\n'
        cmdList.forEach(command => {
         if (similarity(command, cmd) >= 0.01) {
           text += `➤ ${m.prefix}${command} (${this.toPercent(similarity(command, cmd))}\n)`
         }
       })
       return text
     } else {
   ...
   combineMessage(data, data2) {
     let text;
     for (let value of data) {
       if (value.startsWith('_@') && (value.slice(2) === 'command' || value.slice(2) === 'cmd')) {
         text += data2
       } else if (value.startsWith('_@') && (value.slice(2) === 'chat')) {
         text += data2
       } else {
         text += value;
       }
   ...
   __filename(pathURL = fileURLToPath(pathToFileURL('file://' + __filename)), rmPrefix = platform() !== 'win32') {
      ...
      return new Promise(async (resolve, reject) => {
         try {
            if (/^https?:\/\//i.test(string)) {
               ...
               resolve({ data, size, sizeH: this.formatSize(size), ...((await FileType.fromBuffer(data)) || { mime: "application/octet-stream", ext: ".bin" }) });
               ...
               resolve({ data: string, size, sizeH: this.formatSize(size), ...((await FileType.fromBuffer(string)) || { mime: "application/octet-stream", ext: ".bin" }) });
               ...
               resolve({ data: buffer, size, sizeH: this.formatSize(size), ...((await FileType.fromBuffer(buffer)) || { mime: "application/octet-stream", ext: ".bin" }) });
            }
         } catch (e) {
