...
module.exports = async function(axel, msg, config) {
    const m = {};
    const botNumber = axel.decodeJid(axel.user?.id);
    ...
    if (msg.key) {
        m.key = msg.key;
        m.from = axel.decodeJid(m.key.remoteJid);
        ...
        m.sender = axel.decodeJid(
            m.fromMe ? axel.user.id : m.isGroup ? m.participant : m.from
        );
    }
    ...
    m.isOwner =
        ...
        [...config.options.owner, botNumber.split`@`[0]].includes(
            m.senders.replace(/\D+/g, "")
        );

    if (m.isGroup) {
        m.metadata = await axel.groupMetadata(m.from);
    ...
    if (m.message) {
        m.type = axel.getContentType(m.message) || Object.keys(m.message)[0];
    ...
    m.reply = async (text, options = {}) => {
        ...
        if (
            ...
            if (
                ...
                return axel.sendMessage(
                    chatId,
                    {
                        text,
                        mentions: [m.sender, ...axel.parseMention(text)],
                        ...options
                    },
                    { quoted, ephemeralExpiration: m.expiration, ...options }
                );
            } else {
                return axel.sendMedia(m.from, data.data, quoted, {
    ...
    m.download = filepath => {
        if (filepath) return axel.downloadMediaMessage(m, filepath);
        else return axel.downloadMediaMessage(m);
    };
    m.react = icon => {
        axel.sendMessage(m.from, {
            react: {
                text: icon,
                key: m.key
            }
    ...
    m.sendAdMessage = (text, opt) => {
        let ucapan = Func.ucapanWaktu("Asia/Jakarta");
        ...
        let mention =opt && opt.mention != undefined ? opt.mention : axel.parseMention(text);
    ...
    m.sendPayment = (text, amount, types) => {
        ...
        return axel.relayMessage(m.from, {
            requestPaymentMessage: {
                ...
                noteMessage: {
                    extendedTextMessage: {
                        text: text,
                        contextInfo: {
                            mentionedJid: axel.parseMention(text),
                            externalAdReply: {
   ...
   if (m.msg?.contextInfo?.quotedMessage) {
      ...
      if (m.quoted.message) {
         m.quoted.type = axel.getContentType(m.quoted.message) || Object.keys(m.quoted.message)[0]
         m.quoted.msg = extractMessageContent(m.quoted.message[m.quoted.type]) || m.quoted.message[m.quoted.type]
         m.quoted.key = {
            remoteJid: m.msg?.contextInfo?.remoteJid || m.from,
            participant: m.msg?.contextInfo?.remoteJid?.endsWith("g.us") ? axel.decodeJid(m.msg?.contextInfo?.participant) : false,
            fromMe: areJidsSameUser(axel.decodeJid(m.msg?.contextInfo?.participant), axel.decodeJid(axel?.user?.id)),
         ...
         m.quoted.sender = axel.decodeJid(m.msg?.contextInfo?.participant)

         m.quoted.isOwner = m.quoted.sender && [...config.options.owner, botNumber.split`@`[0]].includes(m.quoted.sender.replace(/\D+/g, ""))
         if (m.quoted.isGroup) {
            m.quoted.metadata = await axel.groupMetadata(m.quoted.from)
         ...
         m.quoted.download = (filepath) => {
            if (filepath) return axel.downloadMediaMessage(m.quoted, filepath)
            else return axel.downloadMediaMessage(m.quoted)
         }
      }
    }
  }
